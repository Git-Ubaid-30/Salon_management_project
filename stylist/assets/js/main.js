(function ($) {
    ...
    // Function to set today's date or a future date in the start datetime input
    window.onload = function () {
        var today = new Date();
        var minDateTime = today.toISOString().slice(0, 16); // Get today's date and time in ISO format (YYYY-MM-DDTHH:MM)

        // Set the minimum allowed date/time for the start date to today
        document.getElementById('start_datetime').setAttribute('min', minDateTime);

        // Add an event listener to adjust the min date/time of the end date when the start date is selected
        document.getElementById('start_datetime').addEventListener('change', function () {
            var startDateTime = new Date(this.value);

            // Ensure the end date/time is equal to or after the selected start date/time
            var minEndDateTime = startDateTime.toISOString().slice(0, 16); // Format end datetime to match the format

            // Set the min attribute for the end datetime input field
            document.getElementById('end_datetime').setAttribute('min', minEndDateTime);
        });
    };
